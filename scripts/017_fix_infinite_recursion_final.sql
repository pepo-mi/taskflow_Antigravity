...
-- This script ensures all problematic policies are removed and replaced with safe versions
...
-- USERS TABLE - Simple, non-recursive policies
...
CREATE POLICY "file_attachments_select_all" ON public.file_attachments
  FOR SELECT USING (auth.uid() IS NOT NULL);

CREATE POLICY "file_attachments_insert_own" ON public.file_attachments
  FOR INSERT WITH CHECK (auth.uid() = uploaded_by);

CREATE POLICY "file_attachments_update_uploader" ON public.file_attachments
...
CREATE POLICY "archive_audit_log_select_all" ON public.archive_audit_log
  FOR SELECT USING (auth.uid() IS NOT NULL);
