-- ============================================================================
-- OPTIMIZE ALL RLS AUTH CALLS - Wrap auth functions in SELECT subqueries
-- ============================================================================
-- Per Supabase best practices, all auth.uid() and auth.jwt() calls in RLS
-- policies should be wrapped in (SELECT ...) to prevent per-row re-evaluation
...
DECLARE
    r RECORD;
BEGIN
    RAISE NOTICE 'Dropping all existing RLS policies for optimization...';
    FOR r IN (
...
-- ============================================================================
-- OPTIMIZED HELPER FUNCTION: Check if current user is admin
-- ============================================================================
...
COMMENT ON FUNCTION public.is_admin() IS 'Optimized: Uses (SELECT auth.uid()) to prevent per-row evaluation';

-- ============================================================================
-- USERS TABLE - All auth calls wrapped in SELECT
-- ============================================================================
...
-- ============================================================================
-- GUEST_USERS TABLE - FIXED: Wrap auth.jwt() in SELECT
-- ============================================================================
...
BEGIN
    ...
    WHERE schemaname = 'public'
    AND tablename IN (
        'users', 'workspaces', 'projects', 'tasks', 'posts',
        'file_attachments', 'notifications', 'reactions', 
        'guest_users', 'archive_audit_log'
    );
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'All auth.uid() calls wrapped in (SELECT ...)';
    RAISE NOTICE 'All auth.jwt() calls wrapped in (SELECT ...)';
    RAISE NOTICE 'This prevents per-row re-evaluation';
    RAISE NOTICE '';
    RAISE NOTICE 'Performance improvements:';
    RAISE NOTICE '- Queries will be significantly faster';
    RAISE NOTICE '- Auth functions evaluated once per query';
    RAISE NOTICE '- No more per-row overhead';
    RAISE NOTICE '============================================';
END $$;
